...
端末の物理表示で 38% 程度に縮小されて見える。

**有力仮説**:
Chrome の **サイト別 zoom 状態**が `misfortunemate-png.github.io` に対して 38% 程度で固定されている。
- 復旧手順: Android **設定 → アプリ → Chrome → ストレージ → サイト別ストレージ** から削除
- または USB デバッグで `window.visualViewport.scale` を確認

詳細は Phase 3b 時点の記録を参照。
