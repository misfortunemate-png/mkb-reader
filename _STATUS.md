...
プロジェクト: mkb-reader
最終更新: 2026-05-03（Phase 4 §27+§28 実機検証待ち・§30 実機検証合格）
更新者: PG（Claude Code）

## 現在のフェーズ
**Phase 4 — §27+§28 実装完了（main）実機検証待ち / §30 実機検証合格（feature/vertical）**
...
| Phase 4d | §30（縦書き表示・vertical-rl・スクロール固定） | 合格 |
...
| §30 | 縦書き表示（vertical-rl・スクロール固定） | feature/vertical | 実機検証合格 |
...
- 何を:
  1. `feature/vertical` を `main` にマージ（PM判断）
  2. §27+§28 実機検証（ソート・タグ・ライブラリCRUD・ドリルダウン・タッチDnD）
  3. §29 実装開始（§28合格後）

## §30 実機検証チェックリスト（完了）

- [x] md/txtファイル選択時にチェックボックスが表示される
- [x] mkb/cbz/html/json/画像ではチェックボックスが表示されない
- [x] 縦書きで開くとテキストが右→左方向に縦組みで表示される
- [x] 左方向スワイプで自然に読み進められる
- [x] ページネーションのトグルが非表示になっている
- [x] `npm run build` がエラーなく完了する

## §30 バグ修正メモ（feature/vertical）

- `rehypeTateChuYoko` トランスフォーマーの戻り値が `0` → unified がツリーを数値で置き換えて文章が消えた。`{ processNode(...); }` に修正
- HAST span の `style` 文字列 → React TypeError。`className: ['tcy']` + CSS クラスに変更
- `crypto.randomUUID` は LAN HTTP（非セキュアコンテキスト）で使用不可。`main.jsx` でポリフィル追加
